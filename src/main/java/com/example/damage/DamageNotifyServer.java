...
import com.example.sync.SharedHealthManager; // <-- NOUVEL IMPORT ICI
...
public final class DamageNotifyServer {
    ...
    private static void onAfterDamage(LivingEntity entity, DamageSource source, float baseDamageTaken, float damageTaken,
        ...
        // 1. Affiche le message au-dessus de la barre d'XP pour tout le monde
        broadcast(player, source, damageTaken);
        
        // 👇 2. NOUVEAU : Partage les dégâts avec le reste du serveur si activé ! 👇
        SharedHealthManager.shareDamage(player, damageTaken);
    }
    ...
    private static void onAfterDeath(LivingEntity entity, DamageSource source) {
        ...
        // 👇 NOUVEAU : Si un joueur meurt, on déclenche l'hécatombe ! 👇
        SharedHealthManager.shareDeath(player);
    }
